...
func TestRequestAndHeadersLineParse(t *testing.T) {
	reader := &chunkReader{
	...
	assert.Equal(t, "/hello", r.RequestLine.RequestTarget)
	assert.Equal(t, "1.1", r.RequestLine.HttpVersion)

	_, err = RequestFromReader(strings.NewReader("POST HTTP/1.1"))
	require.Error(t, err)

	_, err = RequestFromReader(strings.NewReader("/hello POST HTTP/1.1"))
	require.Error(t, err)
	...
	require.NoError(t, err)
	require.NotNil(t, r)
	assert.Equal(t, "localhost:42069, another", r.Headers["host"])
	assert.Equal(t, "*/*", r.Headers["accept"])
	...
	require.NoError(t, err)
	require.NotNil(t, r)
	assert.Equal(t, "one, two", r.Headers["host"])
